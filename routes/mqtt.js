...
function connectWithTimeout() {
  setImmediate(() => {
    try {
      // 실제 MQTT 접속 기능을 실행 (내부의 fs.readFileSync가 메인 스레드를 막지 않게 됨)
      console.log('📶 [MQTT] 비동기 백그라운드 스레드에서 연결을 시작합니다.');
      
      // 원래 있던 mqtt 접속 로직 코드들 실행...
      // --- 2. MQTT 연결 옵션 설정 ---
      const options = {
      ...
      console.error('⚠️ [MQTT 예외 발생]: 인증서 디코딩 문제로 가전 알림은 꺼지지만, Gemini 기능은 정상 작동합니다.');
    }
  });
}
...
export { connectWithTimeout as connect };
